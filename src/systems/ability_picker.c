...
void sAbilityPicker(World *world, float dt)
{
    ...
    ComponentMask required_comp =
        (1 << COMPONENT_ABILITY_CONTAINER) | (1 << COMPONENT_CAST_REQUEST) | (1 << COMPONENT_TARGET) | (1 << COMPONENT_AISTATE);
            ...
            if (ability_container->ability_count > 0) {
                for (int j = 0; j < ability_container->ability_count; j++) {
                    if (*(float *)ability_container->remaining_cd.entries[j].value == 0.0f) {
                        Ability current_loop_ability = all_abilities[*(int *)ability_container->abilities.entries[j].value];
                        if (ability_priority < current_loop_ability.priority) {
                            ability_to_cast = current_loop_ability.ability_id;
                        }
                    }
                }
            }
